...
class OwnerLeadController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('manager.owner-leads.index', compact('ownerLeads', 'search', 'status', 'purpose', 'type'));
    }

    public function create()
    {
        return view('manager.owner-leads.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('manager.owner-leads.index')
            ->with('success', $isAr ? 'تم إضافة المالك بنجاح' : 'Owner added successfully');
    }

    public function show(OwnerLead $ownerLead)
    {
        return view('manager.owner-leads.show', compact('ownerLead'));
    }

    public function edit(OwnerLead $ownerLead)
    {
        return view('manager.owner-leads.edit', compact('ownerLead'));
    }
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        if (!empty($ids)) {
            OwnerLead::whereIn('id', $ids)->delete();
        }
        $isAr = app()->getLocale() === 'ar';
        return back()->with('success', $isAr ? 'تم حذف السجلات المحددة' : 'Selected records deleted');
    }

    public function importForm()
    {
        return view('manager.owner-leads.import');
    }
