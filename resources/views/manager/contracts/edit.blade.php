<x-app-layout>
    <x-slot name="title">{{ __('Contracts') }} — {{ __('Edit') }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <h2 class="text-xl font-bold text-gray-800">{{ __('Edit') }} — {{ $contract->title }}</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
        <form method="POST" action="{{ route('manager.contracts.update', $contract) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        <option value="employment" @selected(old('type',$contract->type)==='employment')>{{ __('Employment') }}</option>
                        <option value="service"    @selected(old('type',$contract->type)==='service')>{{ __('Service') }}</option>
                        <option value="freelance"  @selected(old('type',$contract->type)==='freelance')>{{ __('Freelance') }}</option>
                        <option value="supplier"   @selected(old('type',$contract->type)==='supplier')>{{ __('Supplier') }}</option>
                        <option value="other"      @selected(old('type',$contract->type)==='other')>{{ __('Other') }}</option>
                    </select>
                </div>
                <div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Document') }} ({{ __('Optional') }} — {{ __('replaces existing') }})</label>
                @if($contract->document_path)
                <div class="flex gap-2 mb-1">
                    <a href="{{ asset('storage/' . $contract->document_path) }}" target="_blank" class="text-xs text-blue-600 hover:underline">{{ __('Preview') }}</a>
                    <span class="text-gray-300 text-xs">|</span>
                    <a href="{{ asset('storage/' . $contract->document_path) }}" download class="text-xs text-gray-600 hover:underline">{{ __('Download') }}</a>
                </div>
                @endif
