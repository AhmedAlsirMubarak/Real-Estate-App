<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('تعديل المشروع', 'Edit Project') }}: {{ $development->name }}</x-slot>

    <div class="py-4 max-w-4xl">
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('manager.development.show', $development) }}" class="text-gray-400 hover:text-gray-600 transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            </a>
            <h2 class="text-xl font-bold text-gray-900">{{ $tr('تعديل المشروع', 'Edit Project') }}</h2>
        </div>
        ...
        <form method="POST" action="{{ route('manager.development.update', $development) }}" class="space-y-6">
            @csrf @method('PATCH')
            ...
            <div class="bg-white rounded-xl shadow p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" value="{{ old('name', $development->name) }}" required
                    ...
                    <div>
                        ...
                        <select name="type" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
                            <option value="residential" @selected(old('type', $development->type) === 'residential')>{{ $tr('سكني', 'Residential') }}</option>
                            <option value="commercial"  @selected(old('type', $development->type) === 'commercial')>{{ $tr('تجاري', 'Commercial') }}</option>
                            <option value="mixed"       @selected(old('type', $development->type) === 'mixed')>{{ $tr('متعدد الاستخدام', 'Mixed Use') }}</option>
                        </select>
                        @error('type')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('الموقع', 'Location') }} *</label>
                        <input type="text" name="location" value="{{ old('location', $development->location) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="project_manager_name" value="{{ old('project_manager_name', $development->project_manager_name) }}" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
            ...
            <div class="bg-white rounded-xl shadow p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date', $development->start_date?->format('Y-m-d')) }}" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="estimated_completion_date" value="{{ old('estimated_completion_date', $development->estimated_completion_date?->format('Y-m-d')) }}" required
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
                            ...
                            <option value="{{ $val }}" @selected(old('status', $development->status) === $val)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="progress_percentage" value="{{ old('progress_percentage', $development->progress_percentage) }}"
            ...
            @php
                ...
                foreach ($categories as $cat) {
                    ...
                    if ($oldJson) {
                        $initItems = json_decode($oldJson, true) ?: [['name'=>'','amount'=>'']];
                    } else {
                        $initItems = $development->categoryItems($cat);
                        if (empty($initItems)) {
                            $v = (float)$development->categoryBudget($cat);
                            $initItems = [['name'=>'','amount'=>$v > 0 ? $v : '']];
                        }
                    }
                    $catsData[$cat] = $initItems;
                    $openData[$cat] = $development->categoryBudget($cat) > 0;
                }
            ...
            <script>
                function editBudgetComponent() {
                    return {
                        totalBudget: {!! json_encode((float) old('total_budget', $development->total_budget)) !!},
                        open: {!! json_encode($openData) !!},
            ...
            <div class="bg-white rounded-xl shadow p-6 space-y-4" x-data="editBudgetComponent()">
                ...
                <div>
                    ...
                    <input type="number" name="total_budget" x-model="totalBudget"
                           value="{{ old('total_budget', $development->total_budget) }}" step="0.01" min="0" required
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <textarea name="notes" rows="3"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500">{{ old('notes', $development->notes) }}</textarea>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('manager.development.show', $development) }}"
                       class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-xl font-semibold text-sm transition">
                        {{ $tr('إلغاء', 'Cancel') }}
                    </a>
                </div>
                {{-- Delete button id for the separate form below --}}
                <button type="submit" form="delete-project-form"
                        class="text-red-600 hover:text-red-700 text-sm font-medium">
                    {{ $tr('حذف المشروع', 'Delete Project') }}
                </button>
            </div>
        </form>

        {{-- Delete form is OUTSIDE the update form to avoid nested-form HTML bug --}}
        <form id="delete-project-form"
              method="POST" action="{{ route('manager.development.destroy', $development) }}"
              onsubmit="return confirm('{{ $tr('هل أنت متأكد من حذف هذا المشروع نهائياً؟', 'Permanently delete this project?') }}')">
            @csrf @method('DELETE')
        </form>
    </div>
</x-app-layout>
