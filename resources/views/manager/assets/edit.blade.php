<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('أصول الشركة', 'Company Assets') }} — {{ $tr('تعديل', 'Edit') }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <h2 class="text-xl font-bold text-gray-800">{{ $tr('تعديل', 'Edit') }} — {{ $asset->name }}</h2>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        <option value="laptop"           @selected(old('category',$asset->category)==='laptop')>{{ $tr('أجهزة كمبيوتر محمول', 'Laptops') }}</option>
                        <option value="mobile"           @selected(old('category',$asset->category)==='mobile')>{{ $tr('هواتف', 'Mobiles') }}</option>
                        <option value="office_equipment" @selected(old('category',$asset->category)==='office_equipment')>{{ $tr('معدات مكتبية', 'Office Equipment') }}</option>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        <option value="available"    @selected(old('status',$asset->status)==='available')>{{ $tr('متاح', 'Available') }}</option>
                        <option value="assigned"     @selected(old('status',$asset->status)==='assigned')>{{ $tr('مخصص', 'Assigned') }}</option>
                        <option value="under_repair" @selected(old('status',$asset->status)==='under_repair')>{{ $tr('تحت الإصلاح', 'Under Repair') }}</option>
                        <option value="retired"      @selected(old('status',$asset->status)==='retired')>{{ $tr('متقاعد', 'Retired') }}</option>
                    </select>
                </div>
                <div>
