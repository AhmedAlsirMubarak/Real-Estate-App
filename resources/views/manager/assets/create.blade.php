<x-app-layout>
    <x-slot name="title">{{ __('Company Assets') }} — {{ __('Add') }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <h2 class="text-xl font-bold text-gray-800">{{ __('Add') }} — {{ __('Company Assets') }}</h2>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        <option value="laptop"           @selected(old('category')==='laptop')>{{ __('Laptops') }}</option>
                        <option value="mobile"           @selected(old('category')==='mobile')>{{ __('Mobiles') }}</option>
                        <option value="office_equipment" @selected(old('category','office_equipment')==='office_equipment')>{{ __('Office Equipment') }}</option>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        ...
                        <option value="assigned"     @selected(old('status')==='assigned')>{{ __('Assigned') }}</option>
                        <option value="under_repair" @selected(old('status')==='under_repair')>{{ __('Under Repair') }}</option>
