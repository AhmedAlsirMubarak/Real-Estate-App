<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('جمعية الملاك', 'Owners Association') }} — {{ $tr('إضافة', 'Add') }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <h2 class="text-xl font-bold text-gray-800">{{ $tr('إضافة', 'Add') }} — {{ $tr('جمعية الملاك', 'Owners Association') }}</h2>
        ...
        <form method="POST" action="{{ route('manager.associations.store') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="border-t border-dashed border-gray-200 pt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('المخطط', 'Sketch') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('شهادة جمعية الملاك', 'Owners Association Certificate') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('الهوية الشخصية', 'Personal ID') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('هوية مدير الجمعية', "Association Manager's ID") }}</label>
            ...
            <div class="flex gap-2 pt-3">
                ...
                <a href="{{ route('manager.associations.index') }}" class="text-gray-600 px-3 py-2 text-sm">{{ $tr('إلغاء', 'Cancel') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
