...
<head>
<meta charset="utf-8">
<title>شهادة عدم الاعتراض على البيع</title>
<style>
    ...
    /* ── Header ── */
    .hdr        { text-align: center; margin-bottom: 18px; border-bottom: 3px double #0f766e; padding-bottom: 12px; }
    .hdr-logo   { font-size: 18pt; font-weight: bold; color: #0f766e; letter-spacing: 1px; }
    .hdr-sub    { font-size: 9pt; color: #64748b; margin-top: 2px; }
    .cert-title { font-size: 16pt; font-weight: bold; color: #0f766e; text-align: center; margin: 16px 0 4px; }
    .cert-title-en { font-size: 10pt; color: #64748b; text-align: center; margin-bottom: 14px; }

    /* ── Section titles ── */
    .sec        { font-size: 11pt; font-weight: bold; color: #0f766e; border-bottom: 2px solid #0f766e;
    ...
    /* ── Certificate body ── */
    .body-box   { border: 1px solid #99f6e4; background: #f0fdfa; border-radius: 6px;
    ...
    .spacer     { width: 10%; }
    .badge-teal { background: #ccfbf1; color: #0f766e; padding: 1px 9px; border-radius: 9px; font-size: 8.5pt; }
</style>
</head>
<body>
...
<p class="cert-title">شهادة عدم الممانعة على البيع</p>
<p class="cert-title-en">No Objection Certificate for Sale</p>
...
<table class="dtbl">
    ...
    <tr>
        <td class="dlbl">{{ $owner->user?->name ?? '—' }}
            @if($owner->pivot->is_primary) <span class="badge-teal">مالك رئيسي</span> @endif
        </td>
        <td class="dval">
...
{{-- ══ Buyer Data ══ --}}
<div class="sec">بيانات المشتري (الطرف الثاني)</div>
<table class="dtbl">
    <tr>
        <td class="dlbl">الاسم الكامل:</td>
        <td class="dval">{{ $buyer['buyer_name'] }}</td>
        <td class="dlbl">رقم الهاتف:</td>
        <td class="dval">{{ $buyer['buyer_phone'] }}</td>
    </tr>
    <tr>
        <td class="dlbl">رقم الهوية / الإقامة:</td>
        <td class="dval">{{ $buyer['buyer_id'] }}</td>
        <td class="dlbl"></td>
        <td class="dval"></td>
    </tr>
</table>
...
<div class="body-box">
    ...
    بأننا لا نعترض على قيام المالك ببيع وحدته العقارية الكائنة في المبنى المذكور أعلاه
    إلى المشتري
    <span class="highlight">{{ $buyer['buyer_name'] }}</span>
    (رقم هوية: <span class="highlight">{{ $buyer['buyer_id'] }}</span>)،
    وذلك وفق الأنظمة والقوانين المعمول بها في سلطنة عُمان،
    وبشرط أن يلتزم المشتري بنظام الجمعية وجميع الالتزامات المترتبة على الوحدة.
    <br><br>
