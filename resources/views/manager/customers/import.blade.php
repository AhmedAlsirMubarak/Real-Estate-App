...
@endphp
<x-slot name="title">{{ $tr('استيراد عملاء', 'Import Customers') }}</x-slot>
...
<div class="max-w-3xl mx-auto py-4">
    ...
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-800">{{ $tr('استيراد عملاء من CSV', 'Import Customers from CSV') }}</h2>
        <p class="text-sm text-gray-500 mt-1">{{ $tr('ارفع ملف CSV لاستيراد العملاء دفعة واحدة', 'Upload a CSV file to import customers in bulk') }}</p>
    </div>
    ...
    <div class="mb-6 space-y-3">
        ...
        <div class="flex items-start gap-3 bg-green-50 border border-green-200 rounded-xl px-4 py-3">
            ...
            <p class="text-green-800 text-sm font-medium">{{ $tr('تم استيراد ' . $results['imported'] . ' عميل بنجاح.', $results['imported'] . ' customer(s) imported successfully.') }}</p>
        </div>
        @else
        <div class="flex items-start gap-3 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3">
            ...
            <p class="text-yellow-800 text-sm font-medium">{{ $tr('لم يتم استيراد أي عميل.', 'No customers were imported.') }}</p>
        </div>
        @endif
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-4">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-xs text-gray-500 mb-3">{{ $tr('حمّل القالب الذي يحتوي على أسماء الأعمدة الصحيحة. البيانات تبدأ من الصف الثاني.', 'Download the template with correct column headers. Data starts from row 2.') }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                @endif
                <form method="POST" action="{{ route('manager.customers.import') }}" enctype="multipart/form-data"
                      x-data="{ fileName: '' }">
                    @csrf
                    ...
                    <div class="flex gap-3 mt-4">
                        ...
                        <a href="{{ route('manager.customers.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2 rounded-lg text-sm transition">{{ $tr('إلغاء', 'Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
