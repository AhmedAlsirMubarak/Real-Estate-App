<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('الرواتب', 'Salaries') }}</x-slot>

    <div class="mb-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-gray-800">{{ $tr('الرواتب', 'Salaries') }}</h2>
            <p class="text-sm text-gray-500 mt-0.5">{{ $tr('الإجمالي', 'Total') }}: {{ number_format($totalPaid, 2) }}</p>
        </div>
        <div class="flex gap-2">
            ...
            <a href="{{ route('manager.salaries.create') }}" class="bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium">{{ $tr('إضافة', 'Add') }}</a>
        </div>
    </div>

    <div class="bg-white rounded-xl border border-gray-100 p-5 mb-5">
        <h3 class="text-sm font-bold text-gray-800 mb-3">{{ $tr('إنشاء الرواتب الشهرية', 'Generate Monthly Salaries') }}</h3>
        <form method="POST" action="{{ route('manager.salaries.generate') }}" class="flex flex-wrap gap-3 items-end">
            ...
            <button class="bg-blue-900 text-white px-4 py-2 rounded-lg text-sm">{{ $tr('إنشاء الرواتب الشهرية', 'Generate Monthly Salaries') }}</button>
        </form>
    </div>
    ...
    <form method="GET" class="bg-white rounded-xl border border-gray-100 p-4 mb-5 grid grid-cols-1 md:grid-cols-4 gap-3">
        <select name="status" class="border border-gray-200 rounded-lg px-3 py-2 text-sm">
            ...
            <option value="pending" @selected(request('status')==='pending')>{{ $tr('معلق', 'Pending') }}</option>
        ...
        <select name="month" class="border border-gray-200 rounded-lg px-3 py-2 text-sm">
            <option value="">{{ $tr('الشهر', 'Month') }}</option>
            @for($m=1;$m<=12;$m++)<option value="{{ $m }}" @selected(request('month')==$m)>{{ $m }}</option>@endfor
        </select>
        <input type="number" name="year" value="{{ request('year') }}" placeholder="{{ $tr('السنة', 'Year') }}" class="border border-gray-200 rounded-lg px-3 py-2 text-sm">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-600 text-xs uppercase">
                    <tr>
                        <th class="px-4 py-3 text-right">{{ $tr('الموظفون', 'Employees') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('الفترة', 'Period') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('الراتب الأساسي', 'Base Salary') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('إجمالي البدلات', 'Total Allowances') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('المكافآت', 'Bonuses') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('الاستقطاعات', 'Deductions') }}</th>
                        <th class="px-4 py-3 text-right">{{ $tr('صافي المدفوع', 'Net Paid') }}</th>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">
                            ...
                            <span class="text-blue-700 font-medium"
                                  title="{{ $isAr ? 'سكن: ' : 'Housing: ' }}{{ number_format($s->housing_allowance,2) }} | {{ $isAr ? 'مواصلات: ' : 'Transport: ' }}{{ number_format($s->transport_allowance,2) }} | {{ $isAr ? 'طعام: ' : 'Food: ' }}{{ number_format($s->food_allowance,2) }} | {{ $isAr ? 'أخرى: ' : 'Other: ' }}{{ number_format($s->other_allowances,2) }}">{{ number_format($allowances, 2) }}</span>
                            @else
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1">
                                ...
                                <form method="POST" action="{{ route('manager.salaries.pay', $s) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit" title="{{ $tr('صرف الراتب', 'Pay Salary') }}"
                                ...
                                @endif
                                <x-whatsapp-button size="sm" :phone="$s->employee?->phone" :message="$tr('الرواتب', 'Salaries').' '.$s->periodLabel().' — '.number_format($s->net_paid,2)" />
                                <form method="POST" action="{{ route('manager.salaries.destroy', $s) }}" onsubmit="return confirm('{{ $tr('هل أنت متأكد؟', 'Are you sure?') }}')">
                                    @csrf @method('DELETE')
