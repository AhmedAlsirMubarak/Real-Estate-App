<x-app-layout>
    ...
    <div class="mb-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <h2 class="text-xl font-bold text-gray-800">{{ __('Meetings') }}</h2>
        <a href="{{ route('manager.meetings.create') }}" class="bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium">{{ __('Schedule Meeting') }}</a>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-xs">
                        <a href="{{ route('manager.meetings.show', $m) }}" class="text-blue-700 mx-1">{{ __('View Details') }}</a>
                        <a href="{{ route('manager.meetings.edit', $m) }}" class="text-gray-600 mx-1">{{ __('Edit') }}</a>
                    </td>
                </tr>
                @empty
