<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('تعديل', 'Edit') }} — {{ $meeting->title }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <h2 class="text-xl font-bold text-gray-800">{{ $tr('تعديل', 'Edit') }} — {{ $tr('الاجتماعات', 'Meetings') }}</h2>
        <a href="{{ route('manager.meetings.show', $meeting) }}" class="text-sm text-gray-600">{{ $tr('رجوع', 'Back') }}</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-3xl">
        <form method="POST" action="{{ route('manager.meetings.update', $meeting) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                        <option value="scheduled" @selected($meeting->status==='scheduled')>{{ $tr('مجدول', 'Scheduled') }}</option>
                        <option value="completed" @selected($meeting->status==='completed')>{{ $tr('مكتمل', 'Completed') }}</option>
                        <option value="cancelled" @selected($meeting->status==='cancelled')>{{ $tr('ملغى', 'Cancelled') }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('محضر الاجتماع', 'Minutes') }} (AR)</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('محضر الاجتماع', 'Minutes') }} (EN)</label>
            ...
            <div class="flex gap-2 pt-3">
                ...
                <a href="{{ route('manager.meetings.show', $meeting) }}" class="text-gray-600 px-3 py-2 text-sm">{{ $tr('إلغاء', 'Cancel') }}</a>
            </div>
        </form>
    </div>
</x-app-layout>
