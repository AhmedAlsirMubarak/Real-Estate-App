<x-app-layout>
    ...
    @endphp
    <x-slot name="title">{{ $tr('عمولات إدارة المباني', 'Building Management — Business Commission') }}</x-slot>
    ...
    <div class="mb-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                ...
                {{ $tr('عمولة الأعمال — إدارة المباني', 'Business Commission — Building Management') }}
            </h2>
            <p class="text-sm text-gray-500 mt-0.5">{{ $tr('جميع فواتير العمولة الصادرة لعقارات إدارة المباني', 'All commission invoices issued for building management properties') }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        ...
        <div class="bg-indigo-50 rounded-xl border border-indigo-100 shadow-sm p-4">
            <p class="text-xs text-indigo-600">{{ $tr('إجمالي العمولات', 'Total Commissions') }}</p>
            <p class="text-2xl font-bold text-indigo-700 mt-1">{{ number_format($totalCommissions, 3) }} <span class="text-sm font-normal">{{ $tr('ر.ع', 'OMR') }}</span></p>
        </div>
    ...
    <form method="GET" class="bg-white rounded-xl p-4 mb-5 shadow-sm border border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-3">
        <input type="text" name="search" value="{{ $search }}"
               ...
               class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 outline-none md:col-span-2">
        <select name="invoice_for" class="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-400">
        ...
        <div class="md:col-span-4 flex gap-2">
            <button class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm transition">{{ $tr('تصفية', 'Filter') }}</button>
            <a href="{{ route('manager.properties.commissions') }}" class="text-gray-500 text-sm px-3 py-2 hover:text-gray-700">{{ $tr('إعادة تعيين', 'Reset') }}</a>
        </div>
    </form>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-indigo-50/30 transition">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ route('manager.properties.show', $inv->property) }}"
                               class="font-medium text-gray-800 hover:text-indigo-600 text-xs">
                                {{ $inv->property->name }}
                            </a>
                        ...
                        <td class="px-4 py-3">
                            <span class="font-bold text-indigo-700">{{ number_format($inv->commission_amount, 3) }}</span>
                ...
                <tfoot class="bg-gray-50 border-t border-gray-200">
                    <tr>
                        ...
                        <td class="px-4 py-3 font-bold text-indigo-700">{{ number_format($totalCommissions, 3) }} <span class="text-xs font-normal text-gray-400">{{ $tr('ر.ع', 'OMR') }}</span></td>
                        <td></td>
                    </tr>
                </tfoot>
                @endif
