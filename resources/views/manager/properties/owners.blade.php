<x-app-layout>
    <x-slot name="title">{{ __('Owners') }} — {{ $property->name }}</x-slot>

    <div class="mb-5 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800">{{ __('Owners') }} — {{ $property->name }}</h2>
            <p class="text-sm text-gray-500 mt-0.5">{{ $property->code }}</p>
        </div>
        <a href="{{ route('manager.properties.show', $property) }}" class="text-sm text-gray-600 hover:text-gray-900">{{ __('Back') }}</a>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-5 mb-5">
        <div class="flex justify-between items-center mb-3">
            ...
            <span class="text-xs px-2 py-0.5 rounded-full {{ abs($totalPercent - 100) < 0.5 ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                {{ __('Total') }}: {{ number_format($totalPercent, 2) }}%
            </span>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="text-xs text-gray-500 uppercase">
                    <tr>
                        <th class="text-right py-2">{{ __('Name') }}</th>
                        <th class="text-right py-2">{{ __('Ownership %') }}</th>
                        <th class="text-right py-2">{{ __('Primary Owner') }}</th>
                        <th class="text-right py-2">{{ __('Since') }}</th>
                    ...
                    <tr>
                        <form method="POST" action="{{ route('manager.properties.owners.update', [$property, $owner]) }}">
                            ...
                            <td class="py-2 text-xs">
                                <button class="text-blue-700 hover:text-blue-900 mx-1">{{ __('Update') }}</button>
                        </form>
                                <form method="POST" action="{{ route('manager.properties.owners.destroy', [$property, $owner]) }}" class="inline" onsubmit="return confirm('Remove?')">
                                    @csrf @method('DELETE')
                                    <button class="text-red-600 hover:text-red-800 mx-1">{{ __('Delete') }}</button>
                                </form>
                            </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="py-6 text-center text-gray-400 text-xs">{{ __('No data available') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        <h3 class="text-sm font-bold text-gray-800 mb-3">{{ __('Add') }} — {{ __('Owner') }}</h3>
        <form method="POST" action="{{ route('manager.properties.owners.store', $property) }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <input type="number" step="0.01" name="ownership_percentage" required placeholder="{{ __('Ownership %') }}" class="border border-gray-200 rounded-lg px-3 py-2 text-sm">
            <label class="flex items-center gap-2 text-sm text-gray-700"><input type="checkbox" name="is_primary" value="1"> {{ __('Primary Owner') }}</label>
            <button class="bg-blue-900 hover:bg-blue-800 text-white px-4 py-2 rounded-lg text-sm">{{ __('Add') }}</button>
        </form>
