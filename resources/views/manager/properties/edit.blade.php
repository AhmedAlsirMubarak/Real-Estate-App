<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto">
        <a href="{{ route($routePrefix . '.show', $property) }}" class="text-sm text-gray-500 hover:text-gray-700 mb-4 inline-block">← {{ $tr('رجوع', 'Back') }}</a>

        <form method="POST" action="{{ route($routePrefix . '.update', $property) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
            @csrf @method('PATCH')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="commission_notes" rows="2"
                                  ...
                                  class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 focus:border-blue-400 outline-none resize-none">{{ old('commission_notes', $property->commission_notes) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-2 pt-2">
                ...
                <a href="{{ route($routePrefix . '.show', $property) }}" class="bg-gray-100 text-gray-700 px-5 py-2 rounded-lg text-sm">{{ $tr('إلغاء', 'Cancel') }}</a>
            </div>
        </form>
