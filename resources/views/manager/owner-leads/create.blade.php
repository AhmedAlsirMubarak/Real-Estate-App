...
@endphp
<x-slot name="title">{{ $tr('إضافة مالك', 'Add Owner') }}</x-slot>
...
<div class="max-w-3xl mx-auto py-4">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-5">{{ $tr('إضافة مالك جديد', 'Add New Owner') }}</h2>

        <form method="POST" action="{{ route('manager.owner-leads.store') }}">
            @csrf
            @include('manager.owner-leads._form')
