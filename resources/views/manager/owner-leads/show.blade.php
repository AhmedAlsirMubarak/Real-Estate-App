...
@endphp
<x-slot name="title">{{ $ownerLead->name }}</x-slot>
...
<div class="max-w-3xl mx-auto py-4">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <h2 class="text-xl font-bold text-gray-800">{{ $ownerLead->name }}</h2>
                <span class="inline-block mt-2 text-xs font-semibold px-2.5 py-1 rounded-full {{ $ownerLead->statusColor() }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
            <div>
                ...
                <p class="text-sm font-medium text-gray-800">{{ $ownerLead->mobile ?: '—' }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('البريد الإلكتروني', 'Email') }}</p>
                <p class="text-sm font-medium text-gray-800">{{ $ownerLead->email ?: '—' }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('المنطقة / الموقع', 'Area / Location') }}</p>
                <p class="text-sm font-medium text-gray-800">{{ $ownerLead->locationLabel($locale) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('المصدر', 'Source') }}</p>
                @php $sourceLabel = $ownerLead->source ? (\App\Models\OwnerLead::$sources[$ownerLead->source][$locale] ?? $ownerLead->source) : null; @endphp
                @if($sourceLabel)
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800">{{ $ownerLead->created_at?->format('Y/m/d') ?: '—' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('نوع العقار', 'Property Type') }}</p>
                <span class="inline-block bg-blue-50 text-blue-700 text-xs px-2.5 py-1 rounded-full font-medium">
                    {{ $ownerLead->typeLabel($locale) }}
                </span>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('الغرض', 'Purpose') }}</p>
                <span class="inline-block bg-purple-50 text-purple-700 text-xs px-2.5 py-1 rounded-full font-medium">
                    {{ $ownerLead->purposeLabel($locale) }}
                </span>
            </div>
            <div>
                <p class="text-xs text-gray-400 mb-1">{{ $tr('عدد غرف النوم', 'Bedrooms') }}</p>
                <p class="text-sm font-medium text-gray-800">{{ $ownerLead->bedrooms ?: '—' }}</p>
            </div>
            <div>
    ...
    @if($ownerLead->notes)
    <div class="bg-amber-50/60 border border-amber-100 rounded-xl p-6">
        <h3 class="text-sm font-bold text-gray-800 mb-2">{{ $tr('ملاحظات إضافية', 'Additional Notes') }}</h3>
        <p class="text-sm text-gray-600 whitespace-pre-line">{{ $ownerLead->notes }}</p>
    </div>
    @endif
</div>
</x-app-layout>
