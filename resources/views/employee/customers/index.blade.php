...
<div class="py-4">
    ...
    {{-- Search & Filters --}}
    <form method="GET" action="{{ route('employee.customers.index') }}"
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('employee.customers.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm transition">
                {{ $tr('مسح', 'Clear') }}
            </a>
            @endif
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="py-16 text-center">
            ...
            <a href="{{ route('employee.customers.create') }}" class="inline-block mt-3 text-blue-600 hover:underline text-sm">{{ $tr('إضافة أول عميل', 'Add first customer') }}</a>
        </div>
        @else
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap gap-1">
                                ...
                                <span class="bg-purple-50 text-purple-700 text-xs px-2 py-0.5 rounded-full font-medium">{{ $customer->purposeLabel($locale) }}</span>
                                @if($customer->bedrooms)
                                <span class="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full">{{ $customer->bedrooms }} {{ $tr('غرف', 'bed') }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-3 text-gray-700 text-xs">
                            @if($customer->min_budget || $customer->max_budget)
                            {{ $customer->min_budget ? number_format($customer->min_budget) : '' }}{{ ($customer->min_budget && $customer->max_budget) ? ' — ' : '' }}{{ $customer->max_budget ? number_format($customer->max_budget) : '' }}
                            @else —
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            <span class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $customer->statusColor() }}">{{ $customer->statusLabel($locale) }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                @php
