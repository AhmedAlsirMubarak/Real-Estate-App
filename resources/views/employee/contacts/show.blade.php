...
@endphp
<x-slot name="title">{{ $tr('رسالة من', 'Message from') }} {{ $contact->name }}</x-slot>

<div class="py-4 max-w-2xl">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('employee.contacts.index') }}" class="text-gray-500 hover:text-gray-700">
    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-5">
        <div class="flex items-start justify-between">
            <div>
                <h3 class="text-lg font-bold text-gray-900">{{ $contact->subject ?: $tr('بدون موضوع', 'No subject') }}</h3>
        ...
        <div class="border-t border-gray-100 pt-5">
            ...
            <div class="bg-gray-50 rounded-xl p-5 text-gray-700 leading-relaxed text-sm whitespace-pre-line">{{ $contact->message }}</div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3 pt-2">
            ...
            <a href="{{ route('employee.contacts.index') }}" class="text-gray-500 hover:text-gray-700 text-sm px-4 py-2.5">{{ $tr('رجوع', 'Back') }}</a>
        </div>
    </div>
</div>
</x-app-layout>
