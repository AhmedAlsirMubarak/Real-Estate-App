<x-app-layout>
    ...
    <div class="py-4 max-w-3xl">
        ...
        <form method="POST" action="{{ route('employee.tenants.store') }}" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name_ar" value="{{ old('name_ar') }}" required
                               class="w-full border {{ $errors->has('name_ar') ? 'border-red-500' : 'border-gray-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="name_en" value="{{ old('name_en') }}" required
                               class="w-full border {{ $errors->has('name_en') ? 'border-red-500' : 'border-gray-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                        @error('name_en')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('رقم الهاتف', 'Phone') }} <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}"
                               class="w-full border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('كلمة المرور', 'Password') }}</label>
                        <input type="password" name="password"
                               placeholder="{{ $tr('اتركه فارغاً لتوليد كلمة مرور تلقائية', 'Leave blank to auto-generate') }}"
                               class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <select name="unit_id" required class="w-full border {{ $errors->has('unit_id') ? 'border-red-500' : 'border-gray-300' }} rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('تاريخ البدء', 'Start Date') }} <span class="text-red-500">*</span></label>
                        <input type="date" name="start_date" value="{{ old('start_date') }}" required
                               class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('تاريخ الانتهاء', 'End Date') }} <span class="text-red-500">*</span></label>
            ...
            {{-- Referral notice (read-only, rate managed by manager) --}}
            <div class="bg-emerald-50 border border-emerald-100 rounded-xl px-5 py-4 flex items-center gap-2">
                ...
                <p class="text-sm text-emerald-700">{{ $tr('سيتم تسجيل هذا المستأجر كإحالة منك تلقائياً بنسبة عمولتك المحددة من قِبل المدير.', 'This tenant will be automatically attributed as your referral at your commission rate set by the manager.') }}</p>
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition">
                    {{ $tr('حفظ المستأجر والعقد', 'Save Tenant & Contract') }}
                </button>
                <a href="{{ route('employee.dashboard') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg text-sm font-medium transition">
                    {{ $tr('إلغاء', 'Cancel') }}
                </a>
            </div>
        </form>
