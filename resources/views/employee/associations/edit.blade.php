...
@endphp
<x-slot name="title">{{ $tr('تعديل جمعية الملاك', 'Edit Owners Association') }} — {{ $association->name }}</x-slot>

<div class="max-w-3xl mx-auto py-4">
    <a href="{{ route('employee.associations.show', $association) }}"
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-5">{{ $tr('تعديل', 'Edit') }}: {{ $association->name }}</h2>
        ...
        <form method="POST" action="{{ route('employee.associations.update', $association) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf @method('PUT')

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('الاسم', 'Name') }} (AR) <span class="text-red-500">*</span></label>
                    <input type="text" name="name_ar" value="{{ old('name_ar', $association->name_ar) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $association->name_en) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="established_date" value="{{ old('established_date', $association->established_date?->format('Y-m-d')) }}"
            ...
            <div>
                ...
                <select name="status" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none">
                    <option value="active"   @selected(old('status', $association->status) === 'active')>{{ $tr('نشط', 'Active') }}</option>
                    <option value="inactive" @selected(old('status', $association->status) === 'inactive')>{{ $tr('غير نشط', 'Inactive') }}</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea name="description_ar" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none resize-none">{{ old('description_ar', $association->description_ar) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('الوصف (إنجليزي)', 'Description (EN)') }}</label>
                    <textarea name="description_en" rows="3" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none resize-none">{{ old('description_en', $association->description_en) }}</textarea>
                </div>
            </div>
            ...
            <div class="border-t border-dashed border-gray-200 pt-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="electricity_account_number" value="{{ old('electricity_account_number', $association->electricity_account_number) }}"
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $tr('رقم حساب الماء', 'Water Account No.') }}</label>
                        <input type="text" name="water_account_number" value="{{ old('water_account_number', $association->water_account_number) }}"
                               class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-300 outline-none">
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-dashed border-gray-200 pt-4">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">{{ $tr('تحديث المستندات', 'Update Documents') }}</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach([
                        ['no_objection_certificate', 'no_objection_certificate_path', $tr('ملكية', 'Ownership')],
                        ['sketch',                   'sketch_path',                   $tr('المخطط', 'Sketch')],
                        ['association_certificate',  'association_certificate_path',  $tr('شهادة جمعية الملاك', 'Owners Association Certificate')],
                        ['personal_id',              'personal_id_path',              $tr('الهوية الشخصية', 'Personal ID')],
                        ['manager_id',               'manager_id_path',               $tr('هوية مدير الجمعية', "Association Manager's ID")],
                    ] as [$field, $pathField, $label])
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ $label }}</label>
                        @if($association->$pathField)
                        <p class="text-xs text-green-600 mb-1">
                            <svg class="w-3.5 h-3.5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            {{ $tr('مرفوع — حمّل ملفاً جديداً للاستبدال', 'Uploaded — choose new file to replace') }}
                        </p>
                        @endif
            ...
            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('employee.associations.show', $association) }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-lg text-sm font-medium transition">{{ $tr('إلغاء', 'Cancel') }}</a>
                <button type="submit" class="bg-blue-900 hover:bg-blue-800 text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition">{{ $tr('حفظ التعديلات', 'Save Changes') }}</button>
            </div>
        </form>
    </div>
</div>
