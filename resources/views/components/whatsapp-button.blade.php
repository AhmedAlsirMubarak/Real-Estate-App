...
@php
    ...
    if ($normalizedPhone && str_starts_with($normalizedPhone, '0')) {
        $normalizedPhone = '966' . substr($normalizedPhone, 1);
    }
    ...
    $btnLabel = $label ?? __('Send WhatsApp');
@endphp

@if ($href)
    <a href="{{ $href }}" target="_blank" rel="noopener"
       class="inline-flex items-center {{ $classes }} bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition shadow-sm"
       title="{{ $btnLabel }}">
